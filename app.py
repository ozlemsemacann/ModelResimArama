...
ignore_color = st.sidebar.toggle("Renkleri Yoksay (Kalıp/Tip Odaklı)", value=True, 
                               help="Açık olduğunda: Yapay zeka renklere bakmaz, sadece ürünün şekline (yaka, kol boyu, kalıp) odaklanır.")
...
if uploaded_file:
    ...
    with col_left:
        ...
        # Eğer renk yoksayılacaksa, resmi siyah-beyaza çevirip öyle işle
        if ignore_color:
            # Grayscale'e çevir ama model 3 kanal istediği için tekrar RGB yap
            query_img = ImageOps.grayscale(original_img).convert("RGB")
            st.image(original_img, caption="Orijinal", use_container_width=True)
            st.caption("👀 Yapay Zeka (Şekle Odaklanıyor):")
    ...
    with st.spinner("Kalıp ve tip analizi yapılıyor..."):
        if len(filtered_indices) == 0:
            st.error("Filtre sonucunda ürün kalmadı.")
            st.stop()
