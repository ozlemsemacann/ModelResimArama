...
def load_data():
    ...
    if not os.path.exists(embeddings_path):
        return None, None, f"'{embeddings_path}' dosyası bulunamadı! Lütfen proje klasörüne ekleyin."
    if not os.path.exists(excel_path):
        return None, None, f"'{excel_path}' dosyası bulunamadı! Lütfen proje klasörüne ekleyin."

    # 2. Dosya Boyut Kontrolü (Boş dosya hatasını önlemek için)
    if os.path.getsize(embeddings_path) == 0:
        return None, None, f"'{embeddings_path}' dosyası bozuk veya boş (0 byte). Lütfen dosyayı tekrar oluşturun veya indirin."
    if os.path.getsize(excel_path) == 0:
        return None, None, f"'{excel_path}' dosyası bozuk veya boş (0 byte). Lütfen dosyayı tekrar oluşturun veya indirin."
    ...
    try:
        ...
        # "No data left in file" genelde dosya bozuksa çıkar
        return None, None, f"Dosya okuma hatası (Dosyalar bozuk olabilir): {str(e)}"
...
if error_msg:
    ...
    if "openpyxl" in str(error_msg):
        ...
        st.info("Bu hata genellikle 'embeddings.npy' dosyasının içi boşsa veya tam kaydedilmemişse oluşur. Dosyayı yeniden oluşturmayı deneyin.")
    st.stop()
